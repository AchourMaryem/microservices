...
const { HDep, email, numVol } = req.body;
db.run(`INSERT INTO tickets (HDep, email, numVol) VALUES (?, ?, ?)`, [naHDepme,
email, numVol], (err) => {
if (err) {
res.status(400).json({ "error": err.message });
return;
...
app.put('/ticket/:id', (req, res) => {
    const { name, email, numVol } = req.body;
    db.run(`UPDATE tickets SET name = ?, email = ?, numVol = ? WHERE id = ?`,
    [name, email, numVol, req.params.id], (err) => {
    if (err) {
    res.status(400).json({ "error": err.message });
    return;
    }
