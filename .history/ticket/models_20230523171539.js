...
static findById(id, callback) {
    ...
    const ticket = new Ticket(row.HDep, row.HArri, row.password);
    callback(null,ticket);
    });
    }
    // Mettre à jour un utilisateur dans la base de données
    static updateById(id, HDep, HArri, password, callback) {
    db.run(`UPDATE tickets SET HDep = ?, HArri = ?, password = ? WHERE id = ?`,
    [HDep, HArri, password, id], function(err) {
    if (err) {
    console.error(err.message);
    return callback(err);
    }
