...
save(callback) {
db.run(`INSERT INTO tickets (HDep, email, password) VALUES (?, ?, ?)`,
[this.HDep, this.email, this.password], function(err) {
if (err) {
console.error(err.message);
return callback(err);
}
...
const tickets = rows.map(row => new Ticket(row.HDep, row.email,
row.password));
...
static findById(id, callback) {
    ...
    const ticket = new Ticket(row.HDep, row.email, row.password);
    callback(null,ticket);
    });
    }
    // Mettre à jour un utilisateur dans la base de données
    static updateById(id, HDep, email, password, callback) {
    db.run(`UPDATE tickets SET HDep = ?, email = ?, password = ? WHERE id = ?`,
    [HDep, email, password, id], function(err) {
    if (err) {
    console.error(err.message);
    return callback(err);
    }
