...
db.run(`INSERT INTO orders (customerID, ticketId) VALUES (?, ?, ?)`, [customerID,
ticketId], (err) => {
if (err) {
res.status(400).json({ "error": err.message });
return;
...
app.put('/order/:id', (req, res) => {
    const { customerID, ticketId } = req.body;
    db.run(`UPDATE orders SET customerID = ?, ticketId = ?, password = ? WHERE id = ?`,
    [customerID, ticketId, password, req.params.id], (err) => {
    if (err) {
    res.status(400).json({ "error": err.message });
    return;
    }
